...
-- Replaces the 0001 definition. The behaviour is unchanged; only the SQLSTATE
-- codes are now distinct, so the server can answer "this link cannot be used"
-- without also hiding a misconfiguration behind the same message.
--   RD001 the link itself is unusable and the visitor should be told so
--   RD002 the receipt secret no longer derives the stored hash (operator fault)
